...
const rooms = {}; // roomId -> { players: {socketId: {x,y,color,name}} }
...
io.on('connection', (socket) => {
  ...
  socket.on('join_room', ({ roomId, name }) => {
    ...
    // Manda pro novo jogador o estado atual
    socket.emit('room_state', rooms[roomId].players);

    // Avisa os outros que chegou alguém
...
const PORT = 3000;
server.listen(PORT, () => {
  console.log(`\n✅ ArenaPix rodando em http://localhost:${PORT}\n`);
});
